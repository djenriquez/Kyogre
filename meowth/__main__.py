...
async def _wild_internal(message, content):
    ...
    wildreportmsg = await message.channel.send(content=_('Wild {pokemon} reported by {member}! Details: {location_details}').format(pokemon=pkmn.name, member=message.author.mention, location_details=wild_details), embed=wild_embed)
    await asyncio.sleep(0.25)
...
async def _raid_internal(message, content):
    ...
    if not raid_pokemon:
        await message.channel.send(_("I couldn't find the Pokemon name in {content}! Check the spelling and try again!").format(content=content))
        return
    if not raid_pokemon.is_raid:
        await message.channel.send(_('The Pokemon {pokemon} does not appear in raids!').format(pokemon=str(raid_pokemon)))
        return
    ...
    if raidexp is not False:
        if _timercheck(raidexp, raid_info['raid_eggs'][raid_pokemon.raid_level]['raidtime']):
            await message.channel.send(_("That's too long. Level {raidlevel} raids currently last no more than {raidtime} minutes...").format(raidlevel=raid_pokemon.raid_level, raidtime=raid_info['raid_eggs'][raid_pokemon.raid_level]['raidtime']))
            return
    ...
    raidreport = await message.channel.send(content=_('{pokemon} raid reported by {member}! Details: {location_details}. Coordinate in {raid_channel}').format(pokemon=str(raid_pokemon), member=message.author.mention, location_details=raid_details, raid_channel=raid_channel.mention), embed=report_embed)
    ...
    await _update_listing_channels(message.guild, 'raid', edit=False, regions=regions)
    await _send_notifications_async('raid', raid_details, raid_channel, [message.author.id])
    return raid_channel
...
async def _raidegg(message, content):
    ...
    if (egg_level > 5) or (egg_level == 0):
        ...
        raid_embed.set_footer(text=_('Reported by @{author} - {timestamp}').format(author=message.author.display_name, timestamp=timestamp), icon_url=message.author.avatar_url_as(format=None, static_format='jpg', size=32))
        raid_embed.set_thumbnail(url=raid_img_url)
        raidreport = await message.channel.send(content=_('Level {level} raid egg reported by {member}! Details: {location_details}. Coordinate in {raid_channel}').format(level=egg_level, member=message.author.mention, location_details=raid_details, raid_channel=raid_channel.mention), embed=raid_embed)
        await asyncio.sleep(1)
...
async def _eggassume(args, raid_channel, author=None):
    ...
    if not raid_pokemon:
        return
    if not raid_pokemon.is_raid:
        await raid_channel.send(_('The Pokemon {pokemon} does not appear in raids!').format(pokemon=raid_pokemon.name))
        return
    elif raid_pokemon.name.lower() not in raid_info['raid_eggs'][egglevel]['pokemon']:
        await raid_channel.send(_('The Pokemon {pokemon} does not hatch from level {level} raid eggs!').format(pokemon=raid_pokemon.name, level=egglevel))
        return
...
async def _eggtoraid(entered_raid, raid_channel, author=None):
    ...
    if (not raid_match):
        await raid_channel.send(_('The Pokemon {pokemon} does not appear in raids!').format(pokemon=pkmn.name))
        return
    ...
    if str(egglevel) in guild_dict[raid_channel.guild.id]['configure_dict']['counters']['auto_levels'] and not eggdetails.get('pokemon', None):
        ...
        ctrsmessage_id = eggdetails.get('ctrsmessage', None)
    regions = eggdetails.get('region', None)
...
async def research(ctx, *, details = None):
    ...
    while True:
        if details:
            ...
            if not quest:
                return await message.channel.send(_("I couldn't find a quest named '{0}'").format(quest_name))
            reward = await _prompt_reward(ctx, quest)
            if not reward:
                return await message.channel.send(_("I couldn't find a reward for '{0}'").format(quest_name))
...
async def _rewards_add(ctx, *, info):
    ...
    if type.startswith("encounter"):
        ...
        if pokemon:
            pool["encounters"].append(pokemon.name.lower())
    else:
        if not value.isnumeric():
            return await channel.send("Value must be a numeric quantity")
        if type == "stardust":
            pool["stardust"].append(int(value))
        elif type == "xp":
            pool["xp"].append(int(value))
        else:
            pool["items"].setdefault(type, []).append(int(value))
